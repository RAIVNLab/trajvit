...
class Trainer:
    ...
    def _step(
        ...
        outputs = model(batch)
        breakpoint()
        # outputs: list of length frame number. each element is a dict, in which 'pred_masks_high_res' is a tensor of torch.Size([num_object, 1, H, W])
        # each element in tensor is logit, 0 division?
        
        targets = batch.masks
        batch_size = len(batch.img_batch)

        key = batch.dict_key  # key for dataset
        # TODO: calculate loss right here
