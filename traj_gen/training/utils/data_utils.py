...
class BatchedVideoDatapoint:
    ...
    img_batch: torch.FloatTensor  # torch tensor torch.Size([T, B, 3, H, W]) processsed (-2.,2.)
    obj_to_frame_idx: torch.IntTensor # torch tensor ([T, obj_num, 2])   x[...,0]: frame idx    x[...,1]: batch idx the object belongs to
    masks: torch.BoolTensor # torch.tensor ([T, obj_num, H, W]) bool  (I think T can be 1 during evaluation)
    metadata: BatchedVideoMetaData
    # BatchedVideoMetaData(
        # frame_orig_size=Tensor(shape=torch.Size([8, 6, 2]), device=cuda:0, dtype=torch.int64, is_shared=True),
        # unique_objects_identifier=Tensor(shape=torch.Size([8, 6, 3]), device=cuda:0, dtype=torch.int64, is_shared=True),
