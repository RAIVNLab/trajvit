...
def setup_wandb(config, eval=False, manual_version=None):
    if not (config.wandb.enable and is_main_process()):
        return
    if eval: 
        project_name = 'TrajViT_clip_eval'
        version = 'zero_shot_eval:1.0'
    else:
        project_name = 'TrajViT_clip_training'
        version = 'train:1.0'
    
    if manual_version is not None: version = manual_version
